...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y, pc_x, pc_y):
        ...
        self.Delta_T_A = .2 # additional time 
        ...
        print("PolyU-A380 cost part4-> ", self.C_P * (self.Delta_P) )
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            for i, _ in enumerate(self.motion): # tranverse the motion matrix
                ...
                # subtract more cost in minus-cost area
                if self.calc_grid_position(node.x, self.min_x) in self.pc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.pc_y:
                        ...
                        node.cost = node.cost + self.Delta_P * self.motion[i][2] *self.C_P #####MY EDIT
...
def main():
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.plot(pc_x, pc_y, "ob") # plot the minus cost area
