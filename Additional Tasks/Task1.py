...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y):
        ...
        self.Delta_F_A = .3 # additional fuel
    ...
    def planning1(self, sx, sy, c1x, c1y):
        """
        A star path search
            ...
            for i, _ in enumerate(self.motion): # tranverse the motion matrix
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.tc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.tc_y:
                        ...
                        node.cost = node.cost + self.Delta_T_A * self.motion[i][2] *self.C_T #####MY EDIT
                ...
                if self.calc_grid_position(node.x, self.min_x) in self.fc_x:
                    if self.calc_grid_position(node.y, self.min_y) in self.fc_y:
                        ...
                        node.cost = node.cost + self.Delta_F_A * self.motion[i][2] *self.C_F #####MY EDIT
                    # print()
        ...
        rx, ry = self.calc_final_path1(goal_node, closed_set)
        # print(len(closed_set))
        # print(len(open_set))

        return rx, ry

    def planning2(self, c1x, c1y, c2x, c2y):
        """
        A star path search
        ...
        ax, ay = self.calc_final_path2(goal_node, closed_set)
        # print(len(closed_set))
        # print(len(open_set))

        return ax, ay

    def planning3(self, c2x, c2y, gx, gy):
        """
        A star path search
        ...
        bx, by = self.calc_final_path3(goal_node, closed_set)
        # print(len(closed_set))
        # print(len(open_set))

        return bx, by

    def calc_final_path1(self, goal_node, closed_set):
    ...
    def calc_final_path2(self, goal_node, closed_set):
        # generate final course
        ax, ay = [self.calc_grid_position(goal_node.x, self.min_x)], [
            self.calc_grid_position(goal_node.y, self.min_y)] # save the goal node as the first point
        parent_index = goal_node.parent_index
        while parent_index != -1:
            n = closed_set[parent_index]
            ax.append(self.calc_grid_position(n.x, self.min_x))
            ay.append(self.calc_grid_position(n.y, self.min_y))
            parent_index = n.parent_index

        return ax, ay

    def calc_final_path3(self, goal_node, closed_set):
        # generate final course
        bx, by = [self.calc_grid_position(goal_node.x, self.min_x)], [
            self.calc_grid_position(goal_node.y, self.min_y)] # save the goal node as the first point
        parent_index = goal_node.parent_index
        while parent_index != -1:
            n = closed_set[parent_index]
            bx.append(self.calc_grid_position(n.x, self.min_x))
            by.append(self.calc_grid_position(n.y, self.min_y))
            parent_index = n.parent_index
...
def main():
    ...
    for i in range(30, 60): # draw the free border
        ox.append(i)
        oy.append(-4/3 * i + 70)
    ...
    rx, ry = a_star.planning1(sx, sy, c1x, c1y)
    ax, ay = a_star.planning2(c1x, c1y, c2x, c2y)
    bx, by = a_star.planning3(c2x, c2y, gx, gy)
    if show_animation:  # pragma: no cover
